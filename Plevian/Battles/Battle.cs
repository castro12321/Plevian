    ...
    public class Battle
    {
        ...
        private void countAttack()
        {
            foreach (var pair in attacker.getUnits())
            {
                switch (pair.Key)
                {
                    case UnitType.ARCHER: attArch += pair.Value.attackStrength * pair.Value.quantity; break;
                    case UnitType.KNIGHT: attCav += pair.Value.attackStrength * pair.Value.quantity; break;
                    case UnitType.WARRIOR: attInf += pair.Value.attackStrength * pair.Value.quantity; break;
                }
            }
        ...
        private void countDefense()
        {
            foreach (var pair in defender.getUnits())
            {
                defArch += pair.Value.defenseArchers * pair.Value.quantity;  //łucznicy
                defCav += pair.Value.defenseCavalry * pair.Value.quantity;  //kawaleria
                defInf += pair.Value.defenseInfantry * pair.Value.quantity; //piechaota
            }
